...
class GenerateChartUseCase:
    ...
    Orchestrate các domain services và calculators
    """
    ...
    def execute(self, year: int, month: int, day: int, hour: int, 
        ...
        # Convert hour to chi index if needed
        hour_index = None
        if hour is not None:
        ...
        if not has_full_data:
            ...
            interpretation = {
                'summary': 'Lá số thiếu thông tin.',
                'details': ''
            }
        elif calendar_type == 'lunar':
            chart = generate_birth_chart_lunar(
                day, month, year, hour_index, gender, leap_month
            )
            interpretation = generate_overall_interpretation(chart)
        else:  # solar
            chart = generate_birth_chart(
                day, month, year, hour_index, gender
            )
            interpretation = generate_overall_interpretation(chart)
        
        # Use adapter to format response
        response_data = adapter_chart_response(chart, interpretation)
        
        # Add internal chart for AI
